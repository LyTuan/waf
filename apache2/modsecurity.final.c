...
double mean=9.0, variance =3.0,threshold =0.16;
...
static apr_status_t modsecurity_process_phase_request_headers(modsec_rec *msr) {
    ...
    apr_status_t rc = 0;
    char result[1024];
    load_requestpattern("/usr/src/modsecurity/pattern.json", result);
    ...
    url = (char*) malloc(strlen(msr->request_method) +strlen(msr->request_uri) +1);
    strcpy(url, msr->request_method);
    strcat(url, msr->request_uri);
    msr_log(msr,4, "URL:%s", url);
    msr_log(msr,4, "MSR-METHOD:%s", msr->request_method);
    
    msr_log(msr,4, "MSR-HOSTNAME:%s",msr->hostname);
    msr_log(msr,4, "MSR-REMOTE-ADDR:%s", msr->remote_addr);
    
    
    msr_log(msr,4, "server_software:%s", msr->server_software);
    ...
    if(msr->query_string != NULL){
        ...
        for(int i =0; i< arr->nelts; i++){
            ...
            for(h=0; h<num_rp;++h){
                int key_fields_match = 0;
                key_url = node->u.object.keys[h];
                fields = node->u.object.values[h];
                nfields_pattern = fields->u.object.len;
                char *key_url_cd_smd = (char *)malloc(strlen(url)+ strlen("-SMDAnd6BitModel")+1);
                strcpy(key_url_cd_smd , url);
                strcat(key_url_cd_smd,"-SMDAnd6BitModel");
                if(strcmp(key_url, key_url_cd_smd) == 0){
                    for(k=0; k<nfields_pattern; ++k){
                        const char *key_fields = fields->u.object.keys[k];
                        if(strcmp(key_fields, arg_header->name)!=0){
                            key_fields_match = key_fields_match+1;
                        }
                    }
                    if(key_fields_match == nfields_pattern){
                ...
                msr_log(msr, 4, " nfields_pattern: %d, arg_match:%d",nfields_pattern,key_fields_match);
            }
    ...
    if(strcmp(msr->request_method,"GET")==0){
        int url_match = 0;
        for(i=0; i<num_rp; ++i){
            //doi tuong request (json) : method+uri{}
            key_url = node->u.object.keys[i];
            fields = node->u.object.values[i];
            nfields_pattern = fields->u.object.len;
            msr_log(msr, 4, "LYTUAN-KEY_URL: %s", key_url);
            
            char *key_url_metric = (char *)malloc(strlen(url)+ strlen("-LengthModel")+1);
            strcpy(key_url_metric , url);
            strcat(key_url_metric,"-LengthModel");
            if(strcmp(key_url, key_url_metric) == 0){
                for(j=0; j<nfields_pattern; ++j){
                    const char *key_fields = fields->u.object.keys[j];
                    attr = fields->u.object.values[j];
                    size_t num_ele = attr->u.object.len;
                    msr_log(msr, 4, "LYTUAN-KEY_FIELDS: %s", key_fields);
                    field_value = (char *)apr_table_get(msr->request_headers,fields->u.object.keys[j]);
                     int len;
                    if(field_value != NULL){
                        len = strlen(field_value);    
                        for(ii=0; ii<num_ele; ++ii){
                            const char *key_attr = attr->u.object.keys[ii];
                            yajl_val val = attr->u.object.values[ii];
                        
                            if(YAJL_IS_DOUBLE(val)){
                                if(strcmp(key_attr, "mean") == 0){
                                    mean = val->u.number.d;
                                    msr_log(msr, 4,"LYTUAN-MEAN: %lf", mean);
                                }
                                if(strcmp(key_attr,"variance") == 0){
                                    variance = val->u.number.d;
                                    msr_log(msr, 4, "LYTUAN-VARIANCE: %lf", variance);
                                }
                                if(strcmp(key_attr, "threshold") == 0){
                                    threshold = val->u.number.d;
                                    msr_log(msr, 4, "LYTUAN-THRESHOLD: %lf", threshold);
                                }

                            }
                        }
                        msr_log(msr, 4, "FIELD-VALUE: %s:%d - MEAN: %lf", field_value, len, mean);
                        if(len!=mean){
                            double prob = variance/((len-mean)*(len-mean));
                            msr_log(msr, 4, "Probability of %s = %lf", key_fields,prob);
                            if(prob < threshold){
                               //msr->intercept_phase = msr->phase;
                                msr->response_status = HTTP_BAD_REQUEST;
                                //msr->was_intercepted = 1;
                                msr_log(msr, 4, "anormaly");
                                msr_log(msr, 4, "It is an Invalid Request.Deny with code (%d)", HTTP_BAD_REQUEST);
                                return 1;
                            }
                        }
            ...
            //             field_value_cd = distribution(field_value, len);
            //             for(ii=0; ii<num_ele; ++ii){
            //                 const char *key_attr = attr->u.object.keys[ii];
            //                 yajl_val val = attr->u.object.values[ii];
            ...
            //                     if(strcmp(key_attr, "threshold") == 0){
            //                         threshold_cd_smd = val->u.number.d;
            //                         msr_log(msr, 4, "LYTUAN-THRESHOLD: %lf", threshold_cd_smd);
            //                     }

            //                 }
            //                 if(YAJL_IS_ARRAY(val)){
            //                     if(strcmp(key_attr, "deviation") == 0){
            //                         deviation = (double *)calloc(6, sizeof(double));
            //                         size_t len = val->u.array.len;
            //                         int jj;
            //                         for ( jj = 0; jj < len; ++jj ) {
            //                             // get ref to one object in array at a time
            //                             yajl_val obj = val->u.array.values[jj]; // object
            //                             if(YAJL_IS_DOUBLE(obj)){
            //                                     deviation[jj] = obj->u.number.d;
            //                             }
            //                         }
            //                     }
            //                     if(strcmp(key_attr, "mean") == 0){
            ...
            //             for(jj =0; jj< 6; jj++){
            //                 if((field_value_cd[jj] - mean_cd_smd[jj])>=0){
            //                     distance += ((field_value_cd[jj] - mean_cd_smd[jj]))/(deviation[jj] + alpha);    
            //                 }else{
            //                     distance -= ((field_value_cd[jj] - mean_cd_smd[jj]))/(deviation[jj] + alpha);    
            //                 }
            ...
            //             msr_log(msr, 4, "Distance of %s = %lf", key_fields,distance);
            //             if(distance > threshold_cd_smd){
            //                //msr->intercept_phase = msr->phase;
            //                 msr->response_status = HTTP_BAD_REQUEST;
            //                 //msr->was_intercepted = 1;
            //                 msr_log(msr, 4, "anormaly");
        ...
        if(url_match ==  num_rp){
                msr->response_status = HTTP_BAD_REQUEST;
                msr_log(msr, 4, "anormaly because url not in pattern");
                msr_log(msr, 4, "It is an Invalid Request.Deny with code (%d)", HTTP_BAD_REQUEST);
                return 1;
        }   
        ...
        for(int i=0; i< arr->nelts; i++){
            ...
            for(h=0; h<num_rp;++h){
                ...
                if(strcmp(key_url, key_url_cd_smd) == 0){
                    for(k=0; k<nfields_pattern; ++k){
                        const char *key_fields = fields->u.object.keys[k];
                        if(strcmp(key_fields, arg_body->name)!=0){
                            key_fields_match = key_fields_match+1;
                        }
                    }
                    if(key_fields_match == nfields_pattern){
                ...
                msr_log(msr, 4, " arg_match:%d, nfields_pattern: %d",key_fields_match, nfields_pattern);
...
*/ 
   ...
   if(strcmp(msr->request_method,"POST")==0){
            //duyet cac mau request
        int url_match = 0;
        for(i=0; i<num_rp; ++i){
            //doi tuong request (json) : method+uri{}
            key_url = node->u.object.keys[i];
            fields = node->u.object.values[i];
            nfields_pattern = fields->u.object.len;
            msr_log(msr, 4, "LYTUAN-KEY_URL-PHASE-BODY: %s", key_url);
            ...
            if(strcmp(key_url, key_url_metric) == 0){
                for(j=0; j<nfields_pattern; ++j){
                    ...
                    attr = fields->u.object.values[j];
                    size_t num_ele = attr->u.object.len;
                    msr_log(msr, 4, "LYTUAN-KEY_FIELDS-PHASE-BODY: %s", key_fields);
                    field_value = (char *)apr_table_get(msr->request_headers,fields->u.object.keys[j]);
                    int len;
                    if(field_value != NULL){
                        ...
                        for(ii=0; ii<num_ele; ++ii){
                            ...
                            if(YAJL_IS_DOUBLE(val)){
                                if(strcmp(key_attr, "mean") == 0){
                                    mean = val->u.number.d;
                                    msr_log(msr, 4,"LYTUAN-MEAN-PHASE-BODY: %lf", mean);
                                }
                                if(strcmp(key_attr,"variance") == 0){
                                    variance = val->u.number.d;
                                    msr_log(msr, 4, "LYTUAN-VARIANCE-PHASE-BODY: %lf", variance);
                                }
                                if(strcmp(key_attr, "threshold") == 0){
                                    threshold = val->u.number.d;
                                    msr_log(msr, 4, "LYTUAN-THRESHOLD-PHASE-BODY: %lf", threshold);
                                }

                            }
                        }
                        msr_log(msr, 4, "PHASE-BODY-FIELD-VALUE: %s:%d - MEAN: %lf", field_value, len, mean);
                        if(len!=mean){
                            double prob = variance/((len-mean)*(len-mean));
                            msr_log(msr, 4, "Probability of %s = %lf", key_fields,prob);
                            if(prob < threshold){
